...
app.secret_key = os.getenv("FLASK_SECRET_KEY", "dev-secret-key")  # use env in prod

# --------------------------------------------------
# Global Stores (in-memory)
# --------------------------------------------------
processed_documents = {}   # filepath -> document_id
processing_status = {}     # filepath -> status dict
...
def process_document_background(filepath, document_id):
    ...
    try:
        ...
        logger.exception("Exception during background processing")
...
def upload_file():
    try:
        ...
        document_id = f"{Path(filename).stem}_{int(time.time())}"
